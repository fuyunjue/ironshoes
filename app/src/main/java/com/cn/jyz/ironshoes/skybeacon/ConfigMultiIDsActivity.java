...
public class ConfigMultiIDsActivity extends Activity {
	...
	private void setView(SKYBeaconMultiIDs skyBeaconMultiIDs) {
		...
		if (skyBeaconMultiIDs.getLedState() == DefaultStaticValues.DEFAULT_SKY_BEACON_LED_STATE) {
			ledState.setText("不变");
		} else if (skyBeaconMultiIDs.getLedState() == 0) {
			ledState.setText("关闭");
		} else if (skyBeaconMultiIDs.getLedState() == 1) {
			ledState.setText("打开");
		} else if (skyBeaconMultiIDs.getLedState() == 2) {
			ledState.setText("切换");
		}
	}
	...
	public SKYBeaconConfigMultiIDs getView() {
		...
		if (ledState.getText().toString().equals("不变")) {
			...
			skyBeaconConfigMultiIDs.setLedState(0);
		} else if (ledState.getText().toString().equals("打开")) {
			skyBeaconConfigMultiIDs.setLedState(1);
		} else if (ledState.getText().toString().equals("切换")) {
			skyBeaconConfigMultiIDs.setLedState(2);
		}
	...
	public void connectBeacon() {
		...
		SKYBeaconMultiIDs skyBeaconMultiIDs = new SKYBeaconMultiIDs(
				deviceAddress);
		// TODO
		// skyBeaconMultiIDs.setKey("123456");
