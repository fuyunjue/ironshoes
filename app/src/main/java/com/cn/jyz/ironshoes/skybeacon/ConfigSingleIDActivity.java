...
import com.cn.jyz.ironshoes.skybeacon.widget.CheckSwitchButton;
...
public class ConfigSingleIDActivity extends Activity {
	...
	private CheckSwitchButton encryptSB;
	private CheckSwitchButton timeSB;
	private RelativeLayout encryptRL;
	private RelativeLayout timeRL;
	private RelativeLayout ledRL;
	...
	private void initView() {
		...
		if (ledRL != null && ledRL.getVisibility() == View.VISIBLE) {
		...
		encryptSB = (CheckSwitchButton) findViewById(R.id.config_encrypt_switchbutton);
		timeSB = (CheckSwitchButton) findViewById(R.id.config_time_switchbutton);
		encryptRL = (RelativeLayout) findViewById(R.id.config_encrypt);
		timeRL = (RelativeLayout) findViewById(R.id.config_time);
		ledRL = (RelativeLayout) findViewById(R.id.config_led_rl);
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, R.layout.spinner_broadcast_id, R.id.spinner_broadcast_id_view, mUuidList);
		...
		mLedList.add("打开");
		mLedList.add("切换");
		ArrayAdapter<String> adapter1 = new ArrayAdapter<String>(this, R.layout.spinner_broadcast_id, R.id.spinner_broadcast_id_view, mLedList);
	...
	private void setView(SKYBeacon skyBeacon) {
		if (!VersionAdaption.isS4LRRTC(skyBeacon.getHardwareVersion(), skyBeacon.getFirmwareVersionMajor(), skyBeacon.getFirmwareVersionMinor())) {
			encryptRL.setVisibility(View.GONE);
			timeRL.setVisibility(View.GONE);
		} else {
			timeSB.setChecked(false);
			ledRL.setVisibility(View.GONE);
		}
		...
		if (skyBeacon.getMeasuredPower() != DefaultStaticValues.DEFAULT_SKY_BEACON_MEASURED_POWER_FALSE) {
			measuredPower1.setText(String.valueOf(skyBeacon.getMeasuredPower()));
		}
		...
		if (!skyBeacon.getDeviceName().equals(DefaultStaticValues.DEFAULT_SKY_BEACON_DEVICE_NAME)) {
		...
		if (skyBeacon.isEncrypted() == 1) {
			encryptSB.setChecked(true);
		} else {
			encryptSB.setChecked(false);
		}
		// lockSB.setChecked(false);
	...
	public SKYBeaconConfig getView() {
		...
		if (!major1.getText().toString().equals("")) {
			skyBeaconConfig1.setMajor(Integer.valueOf(major1.getText().toString()));
		}
		if (!minor1.getText().toString().equals("")) {
			skyBeaconConfig1.setMinor(Integer.valueOf(minor1.getText().toString()));
		}
		if (!measuredPower1.getText().toString().equals("")) {
			skyBeaconConfig1.setMeasuredPower(Integer.valueOf(measuredPower1.getText().toString()));
		}
		if (!txPower1.getText().toString().equals("")) {
			skyBeaconConfig1.setTxpower(SKYBeaconPower.getPower(Integer.valueOf(txPower1.getText().toString())));
		}
		if (!interval.getText().toString().equals("")) {
			skyBeaconConfig1.setIntervalMillisecond(Integer.valueOf(interval.getText().toString()));
		}
		...
		if (encryptSB.isChecked()) {
			skyBeaconConfig1.setEncrypted(1);
		} else {
			skyBeaconConfig1.setEncrypted(0);
		}
		if (timeSB.isChecked()) {
			skyBeaconConfig1.setTimeConfig(true);
		} else {
			skyBeaconConfig1.setTimeConfig(false);
		}

		if (ledRL != null && ledRL.getVisibility() == View.VISIBLE) {
			if (ledState.getText().toString().equals("不变")) {
				skyBeaconConfig1.setLedState(DefaultStaticValues.DEFAULT_SKY_BEACON_LED_STATE);
	...
	public void connectBeacon() {
		ConnectionStateCallback connectionStateCallback = new ConnectionStateCallback() {
			...
			public void onConnectedSuccess(SKYBeaconMultiIDs skyBeaconMultiIDs) {
				...
				Log.e("bbbbb" , "deviceName: " + skyBeaconMultiIDs.getDeviceName());
			}
			...
			public void onConnectedFailed(SKYBeaconConfigException skyBeaconConfigException) {
				final String exceptionInfo = skyBeaconConfigException.getMessage() + String.valueOf(skyBeaconConfigException.getCode());
				// TODO Auto-generated method stub
				runOnUiThread(new Runnable() {
	...
	private void setDialog(String title, String info, final boolean isFinish) {
		Dialog dialog = new AlertDialog.Builder(this).setTitle(title).setMessage(info).setPositiveButton("确定", new OnClickListener() {
	...
	public void onClick(View v) {
		...
		case R.id.config_command:
			...
			skyBeaconCommunication.configSKYBeacon(skyBeaconConfig, new ConfigCallback() {
				...
				public void onConfigFailed(SKYBeaconConfigException skyBeaconConfigException) {
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							// TODO Auto-generated method
							// stub
							stateInfo.setText("config failed!" + exceptionInfo);
						}
					});
				}
			});
